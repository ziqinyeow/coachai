...
if __name__ == "__main__":
    ...
    # convert one-hot type back to string
    original_val_matches = val_matches.copy()
    original_val_matches['type'] = original_val_matches['type'].apply(lambda x: config['uniques_type'][x-1])
    
    #prepare val_gt.csv
    val_gt = open(f"{config['output_folder_name']}val_gt.csv", "a")
    val_gt.write('rally_id,ball_round,landing_x,landing_y,type,rally_length')
    val_gt.write('\n')
    val_gt.close()

    for rally_id in tqdm(testing_rallies):
        # assign only 4 ball_round in val_matches for each rally_id, while saving 6 consecutive ball_round of them in a file called val_gt.csv
        single_rally = original_val_matches.loc[(original_val_matches['rally_id'] == rally_id)][['rally_id', 'ball_round', 'landing_x', 'landing_y', 'type']].reset_index(drop=True)
        single_rally[4:].to_csv(f"{config['output_folder_name']}val_gt.csv", mode='a', header=False, index=False) 
        
        # read data
        selected_matches = val_matches.loc[(val_matches['rally_id'] == rally_id)][['rally_id', 'type', 'landing_x', 'landing_y', 'player', 'rally_length']].head(4).reset_index(drop=True)
